...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Make sure that the redirectnode is removed from the sitemap by default.
        ct_ids = orm['contenttypes.ContentType'].objects \
            .filter(
                app_label__in=('redirectnode',),
                model__in=('redirectnode',)
            ) \
            .values_list('id', flat=True)

        print "   * Updating RedirectNode, set in_sitemaps = False"
        orm['fluent_pages.UrlNode'].objects.filter(polymorphic_ctype__in=ct_ids).update(in_sitemaps=False)
